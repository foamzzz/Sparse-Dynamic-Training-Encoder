...
class BaseParadigm(metaclass=ABCMeta):
    ...
    def _get_single_subject_data(self, dataset, subject_id, verbose=False):
        ...
        for subject, sessions in data.items():
            for session, runs in sessions.items():
                for run, raw in runs.items():
                    ...
                    channels = (  # 如果是None默认使用全部，不是则使用用户指定的
                        dataset.channels
        ...
        return Xs, ys, metas
        # metas是一个字典，每个key是一个事件，value是一个DataFrame，包含了该事件的所有信息 
    ...
    def get_data(
        ...
        # events, interval checking
        used_events, used_intervals = self._map_events_intervals(dataset) # 获取要处理的事件（event）及其对应的时间区间（interval）
        ...
        X, y, meta = zip(
            *Parallel(n_jobs=n_jobs)(
                delayed(self._get_single_subject_data)(  # 多进程获取每个subject的数据
        ...
        for event_name in used_events.keys():
            for i in range(len(subjects)):
                if event_name in X[i]:
                    # X[i][event_name] 是指 第 i 个被试（subject）在当前事件（event_name）下的 EEG 数据
                    for j in range(len(X[i][event_name])):
                        Xs.append(X[i][event_name][j])

            for i in range(len(subjects)):
                if event_name in y[i]:
                    # y[i][event_name] 是指 第 i 个被试（subject）在当前事件（event_name）下的标签数据
                    for j in range(len(y[i][event_name])):
                        ys.append(y[i][event_name][j])

            if event_name in meta[i]:
                 # meta[i] 是指 第 i 个被试（subject）在当前事件（event_name）下的元信息（meta data）
                 # 其类型为字典，key 是事件名，value 是 pandas 的 DataFrame。
                metas[event_name] = pd.concat(
                    [
                        meta[i][event_name]
                        for i in range(len(subjects))
