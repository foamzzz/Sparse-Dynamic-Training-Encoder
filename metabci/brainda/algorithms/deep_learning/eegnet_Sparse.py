...
class EEGNet_Sparse(nn.Module):
    ...
    EEGNet is a general EEG deep learning model which performs well in alpha BCI paradigms.
    ...
    >>> n_classes = 2
    >>> estimator = EEGNet(X.shape[1], X.shape[2], n_classes)
    ...
    def __init__(self, F1, D, F2, n_channels, n_samples, n_classes, time_kernel_length=64, shared_ratio=0.3, alpha=1.0, epsilon=20):
        ...
        with torch.no_grad():
            ...
            print(f"step3后shape:{fake_output.shape}")


            
        self.classifier = SparseEncoder(fake_output.shape[1], n_classes, self.shared_ratio, self.alpha, self.epsilon)

        # 共享权重现在是 EEGNet 模型直接的可学习参数
        # SparseEncoder 类仍然将决定 *哪些* 权重被使用。
        # 注意形状以匹配 SparseEncoder 中 combined_weight 的期望 (即 mask_scores 的形状)
        self.shared_weights_pool = nn.Parameter(torch.Tensor(n_classes, fake_output.shape[1]))

        nn.init.kaiming_uniform_(self.shared_weights_pool, a=math.sqrt(5))
        # Bias 的初始化现在由 SparseEncoder 内部处理
    ...
    def forward(self, X):
        X = X.unsqueeze(1)  # 4D
        out = self.model(X)
        # 将完整的 shared_weights_pool 传递给 classifier
        # classifier 内部会根据其可学习的 mask_scores 来决定如何使用这些权重
