...
class EnhancedStratifiedKFold(StratifiedKFold):
    ...
    def __init__(
        self,
        n_splits: int = 5,
        shuffle: bool = False, # 是否打乱样本顺序，默认False。
        return_validate: bool = True, # 是否需要验证集，默认True。
        random_state: Optional[Union[int, RandomState]] = None, # 随机种子，保证可复现。
    ):

        self.return_validate = return_validate
        if self.return_validate:  # 是否需要验证集
            # test_size = 1/(n_splits - 1) if n_splits > 2 else 0.5
            test_size = 1 / n_splits # 计算验证集/测试集的比例（每折1/n_splits）。
            self.validate_spliter = StratifiedShuffleSplit(  # 创建StratifiedShuffleSplit 实例用于从训练集中再划分出验证集。
                n_splits=1, test_size=test_size, random_state=random_state
            )
        super().__init__(n_splits=n_splits, shuffle=shuffle, # 调用父类 StratifiedKFold 的构造函数，完成基础初始化。
                         random_state=random_state)

    def split(self, X, y, groups=None): # 重写 split 方法，实现自定义的分层K折交叉验证切分逻辑。
        ...
        update log:
           2023-12-26 by sunchang<18822197631@163.com>
            ...
            """
        for train, test in super().split(X, y, groups=groups): # 调用父类的 split 方法，得到每一折的训练集和测试集索引（train, test）。
            if self.return_validate: # 如果需要验证集
                train_ind, validate_ind = next(  #在当前训练集 train 上再用 self.validate_spliter 划分出训练集和验证集索引。
                    self.validate_spliter.split(X[train], y[train], groups=groups) # 使用StratifiedShuffleSplit类的子函数split，每次划分都保证了类别比例与原始数据一致。
                ) # 在每一折的训练集内部再分层随机划分出训练集和验证集索引，这一步是用分层K折交叉验证（StratifiedKFold）将全部数据分成训练集和测试集，返回的是索引（train, test）。
                yield train[train_ind], train[validate_ind], test  # yield 可以让函数暂停其执行，并在之后的调用中恢复执行，从而节省内存，尤其适用于处理大数据时。
                # return：函数执行到 return 时结束，返回值并退出函数。
                #yield：函数执行到 yield 时暂停，返回当前值，保留当前的状态，下一次调用时从上次暂停的位置继续执行。
            else:
                yield train, test
...
def generate_kfold_indices(
    ...
    """
    subjects = meta["subject"].unique() # 返回唯一值（不重复列表）
    event_names = meta["event"].unique()
    indices = {}
    # 这一段整体来说，数据的划分，不是直接将所有被试的数据拼接后整体划分。是按照一个一个被试（subject）的事件（event）分别处理，最后再将每个被试、每个事件下的划分结果拼接起来。
    # 每次划分都是在同一个被试、同一个事件下的数据上进行的。最后match_kfold_indices拼接时，只是把每个被试、每个事件下的划分结果合并成一个大的训练集/验证集/测试集索引，但每个小块的划分都是独立的。
    for sub_id in subjects:  # 遍历每个被试，存放该被试下每个事件的索引
        sub_ix = meta["subject"] == sub_id
        classes_indices = {}
        for e_name in event_names:  # 遍历每个事件e_name, k_indices存放该事件下的索引
            k_indices = []
            ix = sub_ix & (meta["event"] == e_name) # 布尔数组，表示meta中哪些trial(试次)属于当前被试且属于当前事件。
            spliter = EnhancedStratifiedKFold(  # 创建一个 EnhancedStratifiedKFold 分层K折交叉验证器，指定折数、是否打乱、随机种子。
                n_splits=kfold, shuffle=True, random_state=random_state
            )
            for ix_train, ix_val, ix_test in spliter.split(  # 对当前被试、当前事件下的所有trial(试次)进行K折分层交叉验证。
                np.ones((np.sum(ix))), np.ones((np.sum(ix)))  # 输入（只需要分割，不关心特征内容）
            ):
                k_indices.append((ix_train, ix_val, ix_test)) # 保存返回的训练、验证、测试集的索引（下标）
            classes_indices[e_name] = k_indices
        indices[sub_id] = classes_indices  # 外层key为被试id，内层key为事件名，value为每折的索引三元组列表。
    return indices
...
def match_kfold_indices(k: int, meta: DataFrame, indices):
    ...
    event_names = meta["event"].unique()    
    for sub_id in subjects:                 # 遍历每个被试
        for e_name in event_names:          # 遍历每个事件(40个刺激)
            sub_meta = meta[(meta["subject"] == sub_id) & (meta["event"] == e_name)]  # 选出当前被试和事件下的所有 trial
            train_ix.append(
                sub_meta.iloc[indices[sub_id][e_name][k][0]].index.to_numpy()         # 取出训练集索引并加入列表
            )
            val_ix.append(sub_meta.iloc[indices[sub_id][e_name][k][1]].index.to_numpy())  # 取出验证集索引并加入列表
            test_ix.append(
                sub_meta.iloc[indices[sub_id][e_name][k][2]].index.to_numpy()         # 取出测试集索引并加入列表
            )
    train_ix = np.concatenate(train_ix)     # 将所有被试、事件下的训练集索引拼接成一个一维数组
    val_ix = np.concatenate(val_ix)         # 将所有验证集索引拼接成一个一维数组
    test_ix = np.concatenate(test_ix)       # 将所有测试集索引拼接成一个一维数组
    return train_ix, val_ix, test_ix        

# 保证每个被试、每个事件下的样本都能被均匀、独立地用于训练、验证和测试，避免了被试间数据分布不均带来的影响。
